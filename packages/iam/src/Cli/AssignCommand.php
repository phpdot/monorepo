...
/**
 * iam:assign — write one assignment edge: an identity, a role by id.
 *
...
#[AsCommand(name: 'iam:assign', description: 'Assign a role to an identity.')]
final class AssignCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        try {
            ...
            $this->manager->assign(new AssignmentSaveDTO($identity, $roleId));
            $this->success($output, sprintf(
                'Assigned role [%d] to %s:%s.',
                $roleId,
                $identity->type(),
                (string) ($identity->id() ?? ''),
            ));
